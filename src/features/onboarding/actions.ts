...
export async function createCompanyAction(
  ...
  const parsed = createCompanySchema.safeParse({
    ...
    expected_annual_revenue_manwon: formData.get("expected_annual_revenue_manwon"),
  ...
  await withTransaction(async (client) => {
    ...
    if (!companyId) throw new Error("회사코드 생성 실패. 잠시 뒤 다시 시도하세요.");
