...
- **작성·개정·승인·폐기는 관리자**(관리감독자·안전관리자, 권한은 동일). 작업자 작성과 별도 승인 대기는 두지 않는다. 작성하면 곧 **1판**으로 승인된다(자기 승인, 감사 로그에 남는다)
- **승인된 판은 고치지 않는다.** 고치려면 **개정 시작** → 현재 판을 통째로 복사한 **초안(다음 판 번호)** → 고친 뒤 승인. 초안은 표준서당 하나. 승인 전까지는 기존 판을 그대로 사용하고, 초안은 버릴 수 있다. 폐기하면 남은 초안도 같이 사라진다
- 복사 범위: 작업명, PTW 필요 여부, 작업 단계, 체크리스트, **단계 사진**(같은 파일을 새 단계가 가리킨다). 사진을 붙이거나 지우는 것도 **초안에서만** — 승인된 판은 사진까지 그대로 남는다
- 승인 시 **개정 사유** 한 줄(선택). 상세의 개정 이력에 판·승인일·승인자·사유가 남고, **지난 판은 그대로 읽힌다**
- 표준서 내용: 작업명, 작업 단계·방법, **PTW 필요 여부 체크(Y/N)**
- 표시는 **"n판"** (`STD-0012-V2` 식 버전 ID 없음). 표준서의 이름·PTW 는 현재 판의 거울
- 지시서·위험성평가는 **판 단위**로 연결: 지시서는 표준서를 고를 때 복사한 판을 초안에 들고 있다가 **발급 시 기록**하고(사본에도 판 번호), 평가 회차는 만들 때의 현재 판을 기록한다. 그래서 표준서가 나중에 개정돼도 "그날 그 표준서" 로 돌아간다
...
- **표준서 개정(2판, 3판…)** 뒤 평가는 상세의 "평가 회차 추가" 로 실시한다 (v5.9 구현: 개정 승인이 평가 작성으로 자동 이어지지는 않는다)
...
- 사진 첨부: **선택** (유료 요금제 기능). **항목마다** 붙일 수 있고, 적합·부적합·해당없음
  어느 결과에도 붙는다 — 부적합 증거뿐 아니라 "가드가 닫혀 있다" 같은 적합의 근거도 사진이
...
표준서(마스터)와 판(개정본)을 나눠서 관리. 승인 대기·반려는 없다 — 관리자가 만들고 관리자가 승인한다.
...
| 폐기 | ARCHIVED | 관리자가 폐기. 선택 불가, 개정 불가. 남아 있던 초안은 함께 사라진다. 기존 지시서에는 영향 없음 |
...
| 상태 | 코드 | 조건 |
|---|---|---|
| 작성 중 | DRAFT | 개정 시작으로 현재 판을 복사해 만든 초안. 표준서당 하나. 초안 저장·버리기 가능 |
| 현재 판 | APPROVED | 승인 완료. 고칠 수 없다. 신규 지시서·평가 회차는 이 판을 복사·기록 |
| 지난 판 | SUPERSEDED | 다음 판 승인 시 자동 전환. 그 판을 가리키는 지시서·평가에서 그대로 읽힌다 |

- 초안이 있어도 현재 판은 계속 사용된다. 승인하면 초안 → 현재 판, 현재 판 → 지난 판
