...
v2.3 변경: 작업 전 점검 → TBM으로 명칭 변경(체크리스트 + 작업자 전자서명), PTW 미승인 표시를 작업자 화면에도 노출, 이전 회차 부적합 조치 내용 팝업
...
v0.3 변경: 지시서 승인 개념 삭제(작성자가 바로 발급), PTW 승인 후 지시서 발급으로 흐름 수정, PTW 신청자·부적합 처리 확정, 표준서 개정본 승인 확정
...
- **무료 인원 한도 이하는 무료티어 평생 사용 가능** (작업자 포함 인원, 기본 10명 · 회사별 조정 가능)
- 인원 기준: **현재 활성 인원**
...
| | 무료티어 | Pro티어 |
|---|---|---|
| 대상 | 무료 인원 한도 이하, **평생 무료** (기본 10인) | 한도 초과 시 자동 전환 또는 자발적 전환 |
...
|---|---|---|
| 무료 한도 이하 | 없음 | 무료티어 |
| 한도 초과 (자동 Pro 전환) | **전체 인원** | Pro 기능 개방 |
| 자발적 Pro 전환 | **전체 인원** | Pro 기능 개방 |

- 한도 초과 시 회사 전체가 Pro로 전환되며, 초과한 날부터 과금
- 한도 도달·초과 시 **Pro 업그레이드 안내 표시**
- 자발적 Pro는 10인 이하 회사도 가능

**청구 로직 (일 단위 합산)**

1. 매일 회사별 사용량 1건 기록
   - `daily_usage` = company_id, date, headcount, free_limit, is_pro, billable_count, unit_price
   - `headcount` = 그날 소속 중인 인원 (`joined_at <= date` AND (`left_at` IS NULL OR `left_at >= date`))
   - `is_pro` = 자발적 Pro이거나 headcount > free_limit
   - `billable_count` = is_pro면 headcount, 아니면 0
2. 하루치 금액 = billable_count × (월 요금 ÷ 그 달 일수)
3. 월 청구액 = 그 달 하루치 전부 합산 (원 단위 절사)
4. 월 마감 시 **청구 스냅샷**으로 확정 → 이후 데이터가 바뀌어도 과거 청구서는 불변

예) 한도 10명, 기존 10명, 9/15에 11번째 합류
→ 9/1~9/14: 10명, 한도 이하 → 0원
→ 9/15~9/30(16일): 11명 × (10,000 ÷ 30) × 16일 ≈ **58,666원**

- 인원 중도 가입·퇴사, 한도 아래로 복귀 모두 일 단위로 자동 반영
- 정산 주기: 월 마감 → 익월 청구·결제

### 인원 카운트 기준 (확정)

- **그날과 겹치면 1명으로 카운트** (자정 순간 판정 아님)
  - 조건: `joined_at <= 그날 23:59:59` AND (`left_at` IS NULL OR `left_at >= 그날 00:00:00`)
  - 입사일 · 퇴사일 모두 포함, 하루 안에 입·퇴사해도 그날 1명
- 같은 사람이 하루에 여러 번 들락거려도 **사람 기준 중복 제거**(`COUNT(DISTINCT user_id)`)
- 배치는 매일 새벽에 **전날 사용량**을 계산 (당일 자정 계산 시 저녁 입사자 누락)

### 과금 내역 조회 화면 (확정)

회사 관리자가 과금 산정을 직접 확인할 수 있어야 한다.

- **일자별 인원 리스트**: 날짜별 소속 인원 명단, 그날 인원수, 무료 한도, Pro 여부
- **일자별 금액**: 하루치 단가와 금액, 무료인 날은 0원으로 표시
- **월 합계**: 청구 예정 금액, 확정된 청구 스냅샷 내역
- 인원 변동 이력(입사 · 퇴사 일자)과 청구가 연결되어 보이도록 구성
- 청구서 · 세금계산서 내역 다운로드

### 인원 돌려막기 (확정 — 대응 안 함)
- 매일 번갈아 퇴사·재입사시켜 한도를 피하는 우회가 이론적으로 가능하나, 실효가 낮아 **별도 방어 로직을 두지 않음**
- 실제 그런 패턴이 관찰되면 그때 대응 (월 단위 고유 인원 판정, 단기 재입사 기간 병합 등)
...
- 결제 성공 시 **세금계산서 자동 발행**
- 결제 실패 시: 재시도 후에도 실패하면 **무료티어로 자동 강등** (데이터는 유지, 열람만 제한)
- **100인 이상은 별도 문의(CONTACT) 시 할인 협의**
- **무료 인원 한도는 하드코딩하지 않음**: 회사별 설정값, 기본 10명, 운영자 백오피스에서 변경 가능
...
- **회사별 무료 인원 한도 설정** (기본 10명, 운영자 재량으로 조정)
- 요금제 상태 · 청구 내역 확인
- 100인 이상 할인 등 개별 요금 조건 설정
- 무료티어 모바일은 **현장 기능(지시서 확인 · TBM · 점검)만** 제공. 표준서 열람, 지시서 · PTW 생성, 점검 기록 열람 등 관리 업무는 PC에서만 가능
...
- **작성은 누구나**, **관리감독자 또는 안전관리자 승인** 후 사용
- **개정은 누구나** 가능, 개정 시 버전 생성
- **개정본도 승인 후 적용**, 승인 전까지는 기존 승인 버전 사용
- **폐기는 작성자, 안전관리자, 관리감독자** 가능
- 표준서 내용: 작업명, 작업 단계·방법, **PTW 필요 여부 체크(Y/N)**
- **표준서 ID는 개정할 때마다 버전이 올라감** (예: `STD-0012-V1` → `STD-0012-V2`)
- 지시서·위험성평가는 **버전 단위 ID**로 연결
...
**구조**
- 일반 평가와 간이평가는 **같은 데이터 구조**를 사용하며, 작업표준서 버전 ID는 선택값이다. 표준서 기반 평가는 해당 버전을 연결하고, 간이평가는 연결 없이 작성
- **표준서 버전 1건 : 위험성평가 N건** (최초 1건 + 이후 수시·정기 평가가 누적)
...
- **표준서 개정(V2, V3…) → 같은 방식으로 위험성평가 실시로 이어짐**
- 그 외 사유로 **수시 평가를 언제든 추가** 가능
- 실시 구분은 작성 시 선택 (최초 / 정기 / 수시 / 상시)

**회사 단위 설정 (확정)**
- **위험성 수준 판단 기준**: 기본값 **상 / 중 / 하** 제공, 회사가 기준 문구와 **허용 가능한 위험성 수준**을 수정 가능
...
| 미조치 부적합 | 해당 지시서에 연결된 OPEN 부적합 건수 |

- 표시 예: **"작업기간 종료 · 미조치 2건"**
- 작업기간 종료와 안전조치 완료는 별개. 지시서가 COMPLETED가 되어도 부적합은 자동 종결하지 않음
- 관리자는 기존 부적합 처리 기능에서 조치 내용을 입력해 종결. 지시서의 읽기 전용 상태와 별도로 처리 가능
...
- 작업자 배정은 **기본 복사 + 해제 가능**
- 원본 표준서가 개정됐으면 **표준서에서 온 정보만 최신 버전으로 덮어씀**, 지시서에서 추가·변경한 내용(PTW 포함)은 유지
- 복사 화면에 **표준서 개정 반영 표시** (예: v2 → v3)
- **원본 표준서가 폐기됐으면 복사 금지**
...
### 승인 요청 · 알림 (확정)
- 승인 요청 시 **메일로 승인 링크 발송**, **문자는 Pro티어만**
- 링크 진입 시
  - 승인대기 + 본인이 승인자 → 승인/반려
  - 이미 처리됨 → 결과 표시
...
### 진입 (확정)
- 작업지시서 1건마다 **출력물 생성, QR 코드 인쇄**
- 출력물에 **발행 버전 · 출력일시** 표시, 출력 후 지시서가 바뀌면 점검 화면에 변경 표시
- **작업자, 관리자**가 QR 스캔 → 로그인 → 점검
- 지시서 발행 시 **작업자에게 링크 발송**, 링크로도 TBM 가능
- QR에는 랜덤 토큰 없음
- 권한(같은 회사 + 배정 작업자 또는 관리자)과 회차 판정은 **서버에서 확인**
- **모바일 로그인 상태 장기 유지**
...
- **작업회차** 기준
  ...
  - 회차 판정 범위: **작업 시작 2시간 전 ~ 종료 2시간 후**
- QR 점검은 **현재 작업회차만** 가능
- 전 회차 작업 중 점검을 빠뜨려도 **다음 회차 TBM은 막지 않음**
- 결과 형식: **적합 / 부적합 / 해당없음 + 코멘트**
- 사진 첨부: **선택** (Pro티어 기능)
- 증빙: **진입경로(QR/링크/웹) 기록**
...
1. **표준서 위험요인 · 대책 표시** (해당 작업의 위험을 먼저 공유)
2. **이전 회차 부적합 조치 내용 팝업** — 있으면 우선 노출해 현장 작업자가 재확인
3. **TBM 체크리스트 점검**
4. **확인 체크 후 저장** → 로그인 계정 + 저장 시각이 TBM 참여 증빙 (손글씨 서명 없음)

- 배정 작업자 전원 확인 필요, 미확인자는 누락으로 표시
- **PTW 미승인 상태면 작업자 화면(지시서·TBM)에도 "PTW 미승인" 표시**
...
| 요건 | SMBE 기능 |
|---|---|
| 매월 1회 이상 유해·위험요인 발굴 → 위험성 결정 · 감소대책 수립·실행 | 아차사고 제보, 점검 부적합, 작업 중 점검(순회), 위험성평가 감소대책 |
...
### 티어 (확정)
- **무료티어도 안전점수 전부 제공** (점수, 항목별 현황, 개선 안내 포함)
- 점수를 올리려면 결국 표준서 · 위험성평가 · TBM · 점검을 계속 써야 하므로, 무료 사용자를 활동시키고 Pro로 넘어오게 하는 장치로 사용
...
- 열람 제한 대상은 **작업지시 내역 · 점검 기록** 두 가지뿐
  - 무료티어는 **최근 1주일 이내**만 열람 가능, 이전 건은 목록에 **건수만 표시**
- 표준서, PTW, 회사정보, **발급·진행 중인 지시서**는 기간 제한 없이 열람 가능 (현장 운영이 막히지 않도록)
- Pro 해지 시 무료티어 기준으로 전환 (데이터는 그대로, 열람만 제한)
...
표준서(마스터)와 버전을 나눠서 관리.

**표준서 마스터**

| 상태 | 코드 | 조건 |
|---|---|---|
| 미승인 | UNAPPROVED | 승인된 버전이 하나도 없음. 지시서에서 선택 불가 |
| 사용중 | ACTIVE | 승인된 버전 있음. 지시서에서 선택 가능 |
| 폐기 | DISCARDED | 작성자 또는 관리자가 폐기. 선택 불가, 이 표준서 기반 지시서 복사 금지 |

**표준서 버전**

| 상태 | 코드 | 조건 |
|---|---|---|
| 작성중 | DRAFT | 작성 또는 개정 중 |
| 승인대기 | PENDING | 승인 요청 |
| 반려 | REJECTED | 관리자 반려, 수정 후 재요청 |
| 현재 버전 | CURRENT | 승인 완료. 신규 지시서는 이 버전을 복사 |
| 이전 버전 | SUPERSEDED | 새 버전 승인 시 자동 전환. 기존 지시서 스냅샷은 그대로 유지 |

- 개정본이 승인대기·반려 상태여도 기존 CURRENT 버전은 계속 사용
...
| 상태 | 코드 | 조건 |
|---|---|---|
| 예정 | SCHEDULED | 판정 범위(시작 2시간 전) 이전 |
| 점검 가능 | OPEN | 판정 범위 안. TBM·작업 중 점검 모두 입력 가능 |
| 완료 | DONE | 배정 작업자 전원 TBM 확인 + 작업 중 점검 1건 이상 |
| 누락 | MISSED | 판정 범위(종료 2시간 후) 종료 시점에 미완료. 누락 구분: TBM / 작업 중 / 전체, TBM 누락은 미확인 작업자 표시 |

- 누락 회차를 웹 사후입력으로 채우면 DONE으로 전환, **사후보완 표시** 유지
...
| 상태 | 코드 | 조건 |
|---|---|---|
| 조치대기 | OPEN | 점검 항목 부적합 입력 시 생성, 선택한 관리자에게 알림 |
...
- **개선조치(시정조치) 관리**: 점검 부적합 외 경로(제보, 순회 중 발견, 사고 재발방지대책)도 같은 흐름으로 통합
...
### 3순위 — 운영 · 부가
- 통계 · 지표 (재해율, 점검 이행률, 부적합 처리율) → Pro 대시보드
- 문서함 · 법정 서식 출력 모음
- 공지사항 · 알림
- 결제 · 구독 관리, 고객지원
