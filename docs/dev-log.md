...
- 구간 상승은 즉시(차액 추가 결제), **하락은 다음 결제 주기부터 적용하고 중도 환불은 없다.** 월 중간 등락이 반복되면 정산이 감당되지 않는다.
- **인원 등록은 막지 않는다.** 20인째를 등록할 때 구간 상향을 안내만 하고 등록은 그대로 진행한다. 요금 때문에 등록을 막으면 그 작업자가 시스템 밖에 남아 TBM·점검 기록에 구멍이 생긴다 — 안전관리 도구가 만들면 안 되는 상황이다.
...
- 결제(PG)는 이번 범위가 아니다. 구간 판정·가격 표시·상향 안내까지만 넣었고 `/billing` 은 여전히 문의 폼으로 연결된다. 얼리버드 보호 만료일·적용 단가를 회사별로 저장하는 구독 테이블은 결제 도입 시 함께 만든다(현재는 전역 상수로 판정).
- 코드: `src/features/billing/plans.ts` 신설(구간·가격·할인율·상향 판정), `billing-view.tsx` 가격 섹션 교체, `members-view.tsx` 에 구간 표시와 상향 안내. `PRO_PLAN`(인당 과금 상수) 제거.
- 검증: lint·typecheck·build, DB 회귀 35, 첨부 8, e2e 8, 브라우저 17 전부 통과. 스키마 변경 없음.
